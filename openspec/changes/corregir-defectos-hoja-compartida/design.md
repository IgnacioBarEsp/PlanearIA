...
**Por que.** Reprocesar volveria a llamar `actualizarRecurso` sobre elementos ya escritos, y esa llamada encola una operacion en la cola del motor. Duplicar operaciones en la unica cola sancionada por #84 no cambia el destino final pero ensucia el drenado y contradice el proposito del propio motor. Reanudar tambien hace honesto el conteo: `asignados` acumula a traves de intentos, asi que el resultado final dice cuantos elementos quedaron asignados en total y no cuantos entraron en el ultimo intento.
