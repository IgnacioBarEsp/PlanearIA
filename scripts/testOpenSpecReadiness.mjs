...
assert.equal(report(validateArchive({ root, change: "sample-change", now, runCommand: healthyRun })).ok, false);
writeFileSync(path.join(changeRoot, "tasks.md"), "- [x] done\n");
manifest.exceptions = [{ field: "manual-evidence", reason: "Pendiente temporal", owner: "IgnacioBarEsp", approvedBy: "IgnacioBarEsp", expiresOn: "2026-07-20", recovery: "Adjuntar evidencia antes de archive." }];
...
writeFileSync(path.join(changeRoot, "readiness.json"), JSON.stringify(manifest));
assert.equal(report(validateArchive({ root, change: "sample-change", now, runCommand: healthyRun })).ok, false);
manifest.exceptions = [{ field: "tasks-complete", reason: "No permitido", owner: "IgnacioBarEsp", approvedBy: "IgnacioBarEsp", expiresOn: "2026-07-20", recovery: "Completar tarea." }];
writeFileSync(path.join(changeRoot, "readiness.json"), JSON.stringify(manifest));
assert.equal(report(validateArchive({ root, change: "sample-change", now, runCommand: healthyRun })).ok, false);
